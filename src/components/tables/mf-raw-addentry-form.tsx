...
export default function MfRawAddEntryForm({ size, period, year }: MfRawSizeTableProps) {
...
const handleAddRow = async () => {
  ...
  if (period === "monthly") {
    ...
    const daysToSaturday = (6 - jan1.getUTCDay() + 7) % 7; // if Jan 1 is Mon (1), +5 = Saturday
    const firstSaturday = new Date(jan1);
    firstSaturday.setUTCDate(jan1.getUTCDate() + daysToSaturday);

    // Add (currentCount * 7) days to first Saturday to get the next ones
    period_date = new Date(firstSaturday);
    period_date.setUTCDate(firstSaturday.getUTCDate() + currentCount * 7);
  } else {
