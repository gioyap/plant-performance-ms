...
export default function Size450g600gPage() {
...
const handleExport = () => {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "450g-600g");
  XLSX.writeFile(workbook, "450g-600g.xlsx");
};
  ...
  reader.onload = async (event) => {
    ...
    await Promise.all(
      jsonData.map(async (row) => {
        ...
        const { error } = await supabase
          .from("mf_raw_sizes")
          .upsert({ id, size: "450g-600g", ...rest }, { onConflict: "id" });
  ...
  return (
    <Card className="p-4 w-full overflow-x-auto">
       <div className="flex justify-between items-center mb-4">
          <p className="text-xs italic text-gray-500 dark:text-gray-400 font-semibold">450g-600g</p>
          <DropdownMenu>
