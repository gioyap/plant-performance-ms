...
const handleExport = () => {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "350g-450g");
  XLSX.writeFile(workbook, "350g-450g.xlsx");
};
  ...
  reader.onload = async (event) => {
    ...
    await Promise.all(
      jsonData.map(async (row) => {
        ...
        const { error } = await supabase
          .from("mf_raw_sizes")
          .upsert({ id, size: "350g-450g", ...rest }, { onConflict: "id" });
  ...
  return (
    <Card className="p-4 w-full overflow-x-auto">
       <div className="flex justify-between items-center mb-4">
          <p className="text-xs italic text-gray-500 dark:text-gray-400 font-semibold">350g-450g</p>
          <DropdownMenu>
