...
export default function Size100gBelowPage() {
  ...
  reader.onload = async (event) => {
    ...
    await Promise.all(
      jsonData.map(async (row) => {
        ...
        const { error } = await supabase
          .from("mf_raw_sizes")
          .upsert({ id, size: "100g-below", ...rest }, { onConflict: "id" });
  ...
  return (
    <Card className="p-4 w-full overflow-x-auto">
       <div className="flex justify-between items-center mb-4">
          <p className="text-xs italic text-gray-500 dark:text-gray-400 font-semibold">100g-below</p>
          <DropdownMenu>
